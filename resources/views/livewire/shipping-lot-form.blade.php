<div>
    @if (!$isSubmitted)
        <!-- PROGRESS TRACKER (FIXED NAVBAR STYLE) -->
        <div
            class="sticky-progress sticky bg-white/95 backdrop-blur-md border-b border-gray-200 py-4 px-4 sm:px-6 mb-8 shadow-md">
            <div class="max-w-7xl mx-auto">
                <div class="flex items-center justify-between gap-4 md:gap-10 overflow-x-auto no-scrollbar pb-2 md:pb-0">
                    <!-- Step 1 -->
                    <div class="flex items-center gap-3 min-w-max">
                        <div
                            class="flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all duration-300 {{ $this->isMarchandisesComplete ? 'bg-green-500 border-green-500 text-white shadow-lg shadow-green-500/20' : 'border-blue-500 text-blue-500 bg-white' }}">
                            @if ($this->isMarchandisesComplete)
                        ...
                        <div class="flex flex-col">
                            <span
                                class="text-sm font-black {{ $this->isMarchandisesComplete ? 'text-gray-900' : 'text-blue-600' }}">Marchandise</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 min-w-max">
                        <div
                            class="flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all duration-300 {{ $this->isTermsComplete ? 'bg-green-500 border-green-500 text-white shadow-lg shadow-green-500/20' : 'border-gray-300 text-gray-400 bg-white' }}">
                            @if ($this->isTermsComplete)
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                        d="M5 13l4 4L19 7" />
                                </svg>
                        ...
                        <div class="flex flex-col">
                            <span
                                class="text-sm font-black {{ $this->isTermsComplete ? 'text-gray-900' : 'text-gray-400' }}">Conditions</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 min-w-max">
                        <div
                            class="flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all duration-300 {{ $this->isItineraryComplete ? 'bg-green-500 border-green-500 text-white shadow-lg shadow-green-500/20' : 'border-gray-300 text-gray-400 bg-white' }}">
                            @if ($this->isItineraryComplete)
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                        d="M5 13l4 4L19 7" />
                                </svg>
                        ...
                        <div class="flex flex-col">
                            <span
                                class="text-sm font-black {{ $this->isItineraryComplete ? 'text-gray-900' : 'text-gray-400' }}">Collecte
                                & livraison</span>
                        </div>
                    </div>
                </div>
    ...
    <div class="mx-auto space-y-6">
        @if (!$isSubmitted)
            <!-- SECTION 1: MARCHANDE -->
            <div
                class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] animate-fadeIn shadow-sm">
                <div
                    class="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 class="text-base font-bold text-gray-900">
                            Marchandise
                        </h2>
                        <p class="mt-1 text-sm text-gray-500">Renseignez les détails de votre marchandise.</p>
                    </div>
                        ...
                        <div class="relative overflow-hidden">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                                ...
                                <div class="md:col-span-4">
                                    <label
                                        class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Type
                                        de conditionnement *</label>
                                        ...
                                        <div x-data="{ isOptionSelected: @entangle('palette_type').live ? true : false }" class="relative z-20">
                                            <select wire:model.live="palette_type"
                                                ...
                                                @foreach (array_keys($palette_dimensions) as $p_type)
                                                    <option value="{{ $p_type }}">
                                                        {{ str_replace('_', 'x', $p_type) }} cm</option>
                                                @endforeach
                                            </select>
                                            <span
                                                class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                                ...
                                @elseif($type === 'vehicule')
                                    <div class="md:col-span-4">
                                        <label
                                            class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Marque
                                            *</label>
                                        <input type="text" wire:model.live="brand" placeholder="Peugeot, Honda..."
                                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border {{ $errors->has('brand') ? 'border-red-500' : 'border-gray-300' }} bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                                        @error('brand')
                                            <span
                                    ...
                                    <div class="md:col-span-4">
                                        <label
                                            class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Modèle
                                            *</label>
                                        <input type="text" wire:model.live="model" placeholder="308, CBR1000RR"
                                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border {{ $errors->has('model') ? 'border-red-500' : 'border-gray-300' }} bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                                        @error('model')
                                            <span
                                                class="text-xs text-red-500 mt-1 block font-bold">{{ $message }}</span>
                                        @enderror
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-6 mt-6">
                                @if ($type !== 'vehicule')
                                    @if ($type !== 'palette_standard' && $type !== 'conteneur_complet')
                                        <div class="md:col-span-3">
                                            <label
                                                class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Longueur
                                                (cm) *</label>
                                            <input type="number" wire:model.live="length" placeholder="L"
                                                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border {{ $errors->has('length') ? 'border-red-500' : 'border-gray-300' }} bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                                            @error('length')
                                                <span
                                                    class="text-xs text-red-500 mt-1 block font-bold">{{ $message }}</span>
                                            @enderror
                                        ...
                                        <div class="md:col-span-3">
                                            <label
                                                class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Largeur
                                                (cm) *</label>
                                            <input type="number" wire:model.live="width" placeholder="W"
                                                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border {{ $errors->has('width') ? 'border-red-500' : 'border-gray-300' }} bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                                            @error('width')
                                                <span
                                                    class="text-xs text-red-500 mt-1 block font-bold">{{ $message }}</span>
                                            @enderror
                                    ...
                                    @if ($type !== 'conteneur_complet')
                                        <div class="md:col-span-3">
                                            <label
                                                class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Hauteur
                                                (cm) *</label>
                                            <input type="number" wire:model.live="height" placeholder="H"
                                                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border {{ $errors->has('height') ? 'border-red-500' : 'border-gray-300' }} bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                                            @error('height')
                                                <span
                                                    class="text-xs text-red-500 mt-1 block font-bold">{{ $message }}</span>
                                            @enderror
                                ...
                                @else
                                    <div class="md:col-span-4">
                                        <label
                                            class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Poids
                                            du véhicule *</label>
                                ...
                                <div class="md:col-span-3">
                                    <label
                                        class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Quantité
                                        *</label>
                                    <input type="number" wire:model.live="quantity" min="1"
                                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border {{ $errors->has('quantity') ? 'border-red-500' : 'border-gray-300' }} bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                                    @error('quantity')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-6 mt-6">
                                @if ($type !== 'vehicule')
                                    <div class="md:col-span-12 lg:col-span-6">
                                        <label class="mb-1.5 block text-sm font-medium text-slate-500">Poids
                                            unitaire (kg) *</label>
                                        <div class="relative">
                                            <input type="number" wire:model.live="weight" step="0.01"
                                                class="h-11 w-full rounded-xl border {{ $errors->has('weight') ? 'border-red-500' : 'border-gray-300' }} bg-white pl-4 pr-12 text-sm text-gray-800 font-medium focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all shadow-sm" />
                                            <div
                                                class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                                <span
                                                    class="text-gray-400 text-sm font-black uppercase tracking-wider">kg</span>
                                            </div>
                                        </div>
                                        @error('weight')
                                            <span
                                    ...
                                    <div class="md:col-span-12 lg:col-span-6 flex flex-col justify-end pb-2.5">
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" wire:model.live="is_stackable"
                                                class="sr-only peer">
                                            <div
                                                class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-brand-500/10 dark:peer-focus:ring-brand-500/20 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-brand-500">
                                            </div>
                                            <span
                                                class="ml-3 text-sm font-medium text-gray-700 dark:text-gray-300">Superposable</span>
                                        </label>
                                    </div>
                                @else
                                    <div class="md:col-span-8 flex flex-wrap gap-6 items-end pb-2.5">
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" wire:model.live="is_rolling" class="sr-only peer">
                                            <div
                                                class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-brand-500/10 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-brand-500">
                                            </div>
                                            <span
                                                class="ml-3 text-sm font-medium text-gray-700 dark:text-gray-300">Véhicule
                                                roulant</span>
                                        </label>
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" wire:model.live="is_stackable"
                            ...
                            @if ($type !== 'vehicule')
                                <div class="mt-8 pt-8 border-t border-gray-100 dark:border-gray-800">
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <div class="relative">
                                                <input type="text" readonly
                                                    value="{{ number_format((float) ($weight ?: 0) * (int) ($quantity ?: 1), 2, '.', '') }}"
                                                    class="h-11 w-full rounded-xl border border-gray-300 bg-white pl-4 pr-12 text-sm text-gray-800 font-medium outline-none shadow-sm select-none" />
                                                <div
                                                    class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                                    <span
                                                        class="text-gray-400 text-sm font-black uppercase tracking-wider">kg</span>
                                        ...
                                        <!-- Volume unitaire -->
                                        <div>
                                            <label class="mb-1.5 block text-sm font-medium text-slate-500">Volume
                                                unitaire</label>
                                            <div class="relative">
                                                <input type="text" readonly
                                                    value="{{ number_format($this->unit_volume, 4, '.', '') }}"
                                                    class="h-11 w-full rounded-xl border border-gray-300 bg-white pl-4 pr-12 text-sm text-gray-800 font-medium outline-none shadow-sm select-none" />
                                                <div
                                                    class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                                    <span
                                                        class="text-gray-400 text-sm font-black lowercase tracking-wider">m³</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="relative">
                                                <input type="text" readonly
                                                    value="{{ number_format($this->current_lot_volume, 4, '.', '') }}"
                                                    class="h-11 w-full rounded-xl border border-gray-300 bg-white pl-4 pr-12 text-sm text-gray-800 font-medium outline-none shadow-sm select-none" />
                                                <div
                                                    class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                                    <span
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 flex items-center gap-3">
                        <div class="flex items-center h-5">
                            <input type="checkbox" wire:model.live="termsAccepted" id="termsAccepted"
                                class="h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer transition-all">
                        </div>
                        <div class="ml-1 text-sm">
                            <label for="termsAccepted" class="font-bold text-gray-800 cursor-pointer select-none">
                                J'ai renseigné toutes les conditions et je valide cette étape.
                            </label>
                            <p class="text-gray-500 text-xs mt-0.5">Cette étape sera marquée comme complète une fois
                                cochée.</p>
                        </div>
                    </div>
                </div>

                <!-- SECTION 3: COLLECTE + LIVRAISON  -->
                <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm animate-fadeIn">
                    <div class="mb-5">
                    ...
                    <div
                        class="mb-8 grid grid-cols-1 md:grid-cols-3 gap-6 p-4 bg-indigo-50/30 rounded-xl border border-indigo-100/50">
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2">Validité de la demande (Date)
                                *</label>
                            <div x-data="{ value: @entangle('validityDate').live }" x-init="flatpickr($refs.datepicker, { dateFormat: 'Y-m-d', defaultDate: value, onChange: function(selectedDates, dateStr, instance) { value = dateStr; } })">
                                <div class="relative">
                                    <input type="text" x-ref="datepicker" x-model="value"
                                        placeholder="AAAA-MM-JJ"
                                        class="h-11 w-full appearance-none rounded-xl border {{ $errors->has('validityDate') ? 'border-red-500' : 'border-gray-300' }} bg-white px-4 pr-11 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                                    <span
                                        class="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 pointer-events-none">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="time" wire:model.live="validityTime" onclick="this.showPicker()"
                                    class="h-11 w-full appearance-none rounded-xl border {{ $errors->has('validityTime') ? 'border-red-500' : 'border-gray-300' }} bg-white px-4 pr-11 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                                <span
                                    class="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 pointer-events-none">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" wire:model.live="deliveryPrice"
                                    placeholder="0.00"
                                    class="h-11 w-full pl-8 rounded-xl border {{ $errors->has('deliveryPrice') ? 'border-red-500' : 'border-gray-300' }} bg-white px-4 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                            </div>
                            @error('deliveryPrice')
                                <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span>
                            @enderror
                                ...
                                <div class="space-y-4">
                                    <div class="relative">
                                        <input type="text" wire:model.live="pickupAddress"
                                            ...
                                            class="block w-full h-11 bg-white border {{ $errors->has('pickupAddress') ? 'border-red-500' : 'border-gray-300' }} rounded-xl px-4 text-sm focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors pl-12">
                                    ...
                                    <div class="border border-gray-200 rounded-xl p-4">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            <div x-data="{ value: @entangle('latestPickupDate').live }" x-init="flatpickr($refs.datepicker, { dateFormat: 'Y-m-d', defaultDate: value, onChange: function(selectedDates, dateStr, instance) { value = dateStr; } })">
                                                <div class="relative">
                                                    <input type="text" x-ref="datepicker" x-model="value"
                                                        placeholder="AAAA-MM-JJ"
                                                        class="h-11 w-full appearance-none rounded-xl border {{ $errors->has('latestPickupDate') ? 'border-red-500' : 'border-gray-300' }} bg-white px-4 pr-11 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                                                    <span
                                                        class="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 pointer-events-none">
                                            ...
                                            <div>
                                                <div class="relative">
                                                    <input type="time" wire:model.live="latestPickupTime"
                                                        ...
                                                        class="h-11 w-full appearance-none rounded-xl border {{ $errors->has('latestPickupTime') ? 'border-red-500' : 'border-gray-300' }} bg-white px-4 pr-11 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                                                    <span
                                                        class="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 pointer-events-none">
                            ...
                            <div class="space-y-4">
                                <div class="relative">
                                    <input type="text" wire:model.live="deliveryAddress"
                                        ...
                                        class="block w-full h-11 bg-white border {{ $errors->has('deliveryAddress') ? 'border-red-500' : 'border-gray-300' }} rounded-xl px-4 text-sm focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors pl-12">
                                ...
                                <div class="border border-gray-200 rounded-xl p-4">
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div x-data="{ value: @entangle('latestDeliveryDate').live }" x-init="flatpickr($refs.datepicker, { dateFormat: 'Y-m-d', defaultDate: value, onChange: function(selectedDates, dateStr, instance) { value = dateStr; } })">
                                            <div class="relative">
                                                <input type="text" x-ref="datepicker" x-model="value"
                                                    placeholder="AAAA-MM-JJ"
                                                    class="h-11 w-full appearance-none rounded-xl border {{ $errors->has('latestDeliveryDate') ? 'border-red-500' : 'border-gray-300' }} bg-white px-4 pr-11 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                                                <span
                                                    class="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 pointer-events-none">
                                                    <svg class="w-[18px] h-[18px]" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div>
                                            <div class="relative">
                                                <input type="time" wire:model.live="latestDeliveryTime"
                                                    onclick="this.showPicker()"
                                                    class="h-11 w-full appearance-none rounded-xl border {{ $errors->has('latestDeliveryTime') ? 'border-red-500' : 'border-gray-300' }} bg-white px-4 pr-11 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                                                <span
                                                    class="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 pointer-events-none">
                                                    <svg class="fill-current" width="18" height="18"
                                                        viewBox="0 0 20 20" fill="none"
