...
@section('content')
    ...
    <div
        ...
        <div class="mb-6">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Edit Language</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400">Update code, name, and status.</p>
        </div>
        ...
        <form method="POST" action="{{ route('dashboard.localization.languages.update', $language) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-400">
                <input type="checkbox" name="is_active" value="1"
                    {{ old('is_active', (int) $language->is_active) ? 'checked' : '' }}>
                Active
            </label>
