...
@section('content')
    ...
    <div
        ...
        <div class="mb-6">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Edit Key</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400">Update group and key identifier.</p>
        </div>
        ...
        <form method="POST" action="{{ route('dashboard.localization.keys.update', $translationKey) }}" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    Group <span class="text-error-500">*</span>
                </label>
                <input type="text" name="group" value="{{ old('group', $translationKey->group) }}"
            ...
            <div>
                ...
                <input type="text" name="key" value="{{ old('key', $translationKey->key) }}"
                    class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                    required />
                @error('key')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('dashboard.localization.translations.edit', $translationKey) }}"
                    class="ml-auto inline-flex items-center justify-center rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/5">
                    Translate
                </a>
            </div>
        </form>
