...
@section('content')
    ...
    <div
        ...
        <div class="mb-6">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">New Key</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400">Create a new translation key and assign a group.</p>
        </div>
        ...
        <form method="POST" action="{{ route('dashboard.localization.keys.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="group" value="{{ old('group') }}" placeholder="e.g. auth, profile, common"
            ...
            <div>
                ...
                <input type="text" name="key" value="{{ old('key') }}" placeholder="e.g. auth.login.title"
                    class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                    required />
                @error('key')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
