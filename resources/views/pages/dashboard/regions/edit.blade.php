...
@section('content')
    <div class="mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Region</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update region information and translations.</p>
            </div>
        </div>
    </div>

    <form method="POST" action="{{ route('dashboard.regions.update', $region->id) }}" class="space-y-6">
        @method('PUT')
        @include('pages.dashboard.regions._form')
