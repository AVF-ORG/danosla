            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Total control over your business sectors and their translations.
                    </p>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <div class="flex items-center rounded-lg border border-gray-200 bg-white p-1 dark:border-gray-800 dark:bg-gray-900">
                        ...
                        <a href="{{ route('dashboard.sectors.index', ['trash' => 1]) }}"
                            class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors {{ request('trash') ? 'bg-brand-500 text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300' }}">
                            Trash
                        </a>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h4 class="mt-2 text-2xl font-bold text-gray-900 dark:text-white">{{ $sectors->total() }}</h4>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h4 class="mt-2 text-2xl font-bold text-gray-900 dark:text-white">{{ $sectors->where('is_active', true)->count() }}</h4>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h4 class="mt-2 text-2xl font-bold text-gray-900 dark:text-white">{{ $sectors->where('deleted_at', '!=', null)->count() }}</h4>
                    </div>
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="border-y border-gray-100 dark:border-white/[0.05]">
                                ...
                                <th class="px-6 py-4 text-center text-theme-xs font-semibold uppercase tracking-wider text-gray-500">Sector Name</th>
                                ...
                                <tr class="group transition-colors hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex -space-x-2">
                                            @foreach ($sector->getTranslations('name') as $code => $val)
                                    ...
                                    <td class="px-6 py-4 text-theme-sm text-gray-500">
                                        {{ optional($sector->created_at)->format('M d, Y') }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div x-data="{ dropdownOpen: false, openDeleteModal: false, isForceDelete: false }" class="relative inline-block text-left w-full" @click.away="dropdownOpen = false">
                                            ...
                                            <div x-show="dropdownOpen" 
                                                 ...
                                                 style="display: none;">
                                                <div class="py-1">
                                                    <a href="{{ route('dashboard.sectors.show', $sector) }}" class="flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" onclick="event.stopPropagation();">
                                                        <svg class="w-4 h-4 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                                        Voir Détails
                                                    </a>
                                                    @if (!request('trash'))
                                                        <a href="{{ route('dashboard.sectors.edit', $sector) }}" class="flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" onclick="event.stopPropagation();">
                                                    ...
                                                    @else
                                                        <a href="{{ route('dashboard.sectors.restore', $sector->id) }}" class="flex items-center px-4 py-2.5 text-sm text-success-600 hover:bg-success-50 dark:hover:bg-success-900/20 transition-colors" onclick="event.stopPropagation();">
                                                ...
                                                <div @click.stop>
                                                    <x-ui.modal model="openDeleteModal" title="Confirmation" maxWidth="max-w-md">
                                                        <div class="sm:flex sm:items-start whitespace-normal">
                                                            ...
                                                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                                                <p class="text-sm text-gray-500 dark:text-gray-400 break-words">
                                                                    <span x-show="!isForceDelete">Êtes-vous sûr de vouloir supprimer le secteur <strong>{{ $sector->name }}</strong> ?</span>
                                                                    <span x-show="isForceDelete">Êtes-vous sûr de vouloir supprimer **définitivement** le secteur <strong>{{ $sector->name }}</strong> ? Cette action est irréversible.</span>
                                                                </p>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <x-slot:footer>
                                                            ...
                                                            <form :action="isForceDelete ? '{{ route('dashboard.sectors.forceDelete', $sector->id) }}' : '{{ route('dashboard.sectors.destroy', $sector->id) }}'" method="POST" class="inline">
                                                                @csrf
                                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-gray-500 dark:text-gray-400 font-medium">No sectors found</p>
                                            <p class="text-gray-400 text-sm mt-1">Try changing your filters or add a new sector.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
