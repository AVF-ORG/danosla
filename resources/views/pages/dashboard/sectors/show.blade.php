...
@section('content')
    <div class="mb-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Sector Details</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Comprehensive overview of the sector and its translations.</p>
            </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white">#{{ $sector->id }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Primary Name</p>
                        <p class="mt-1 text-lg font-semibold text-brand-500">{{ $sector->name }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Created At</p>
                        <p class="mt-1 text-sm text-gray-700 dark:text-gray-300">{{ optional($sector->created_at)->format('M d, Y H:i') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <p class="mt-4 text-xs text-gray-500 dark:text-gray-400 italic">
                    {{ $sector->is_active ? 'Visible to all users on the frontend.' : 'Hidden from the public frontend.' }}
                </p>
            </div>
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-100 dark:divide-white/[0.05]">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/[0.05]">
                            @forelse ($sector->getTranslations('name') ?? [] as $code => $value)
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                                {{-- You might have a helper to get language name from code --}}
